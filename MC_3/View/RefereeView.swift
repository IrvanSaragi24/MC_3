...
struct RefereeView: View {
    ...
    @State private var votingStatusText = ""
                    ...
                    ZStack {
                        ZStack {
                            Text(votingStatusText)
                                ...
                                .padding(.bottom, 170)
                                .onAppear {
                                    if vibrateOnRing || vibrateOnRing1 {
                                        votingStatusText = "Wait for referees to vote \n" +
                                            "   Voting : \(multipeerController.totalVote)/\(multipeerController.getConnectedPeers().count)\(dots)"
                                    } else {
                                        votingStatusText = "Judge The \n Player"
                                    }
                                    animateDots()
                                }
                            ButtonSliderReferee(
                                ...
                                vibrateOnRing1: $vibrateOnRing1,
                                message: .constant("No:VoteStatus"))
                            ...
                            ButtonSliderReferee(
                                ...
                                vibrateOnRing1: $vibrateOnRing,
                                message: .constant("Yes:VoteStatus"))
                                .rotationEffect(Angle(degrees: -90))
