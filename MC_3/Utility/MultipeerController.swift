...
//    @Published var connectedGuest: [MCPeerID] = [] // Do not use this var
    ...
    // Please don't use this, All lobby related stuff should be in separated View Model which is LobbyViewModel
...
extension MultipeerController: MCNearbyServiceBrowserDelegate {
    func browser(
        ...
        if let matchingGuest = allGuest.first(where: { $0.id == peerID }) {
            // Access the status of the matching guest
            _ = matchingGuest.status
